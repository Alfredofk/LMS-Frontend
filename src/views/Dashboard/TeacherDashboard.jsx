    ...
    <div className="space-y-6 text-left">
      ...
      <div className="select-none text-left">
        <h1 className="text-3xl font-black text-slate-900 tracking-tight leading-tight capitalize">
          Selamat {getGreeting()}, Bapak/Ibu {user?.name || 'Guru'}
        </h1>
        <p className="text-xs sm:text-sm text-slate-905 font-black mt-1">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-5 select-none">
        ...
        <div className="bg-white border border-slate-100 rounded-2xl p-6 shadow-sm flex items-center justify-between hover:shadow-md transition-shadow duration-200">
          <div className="space-y-1">
            <span className="text-xs font-black text-slate-400 uppercase tracking-wider">Total Kelas</span>
            <h3 className="text-3xl font-black text-slate-805">{dashboardData.stats.totalClasses}</h3>
          </div>
          <div className="w-12 h-12 rounded-xl bg-purple-50 text-[#7047EB] flex items-center justify-center shrink-0">
        ...
        <div className="bg-white border border-slate-100 rounded-2xl p-6 shadow-sm flex items-center justify-between hover:shadow-md transition-shadow duration-200">
          <div className="space-y-1">
            <span className="text-xs font-black text-slate-400 uppercase tracking-wider">Total Siswa</span>
            <h3 className="text-3xl font-black text-slate-805">{dashboardData.stats.totalStudents}</h3>
          </div>
        ...
        <div className="bg-amber-50/50 border border-amber-100 rounded-2xl p-6 shadow-sm flex items-center justify-between hover:shadow-md transition-shadow duration-200">
          <div className="space-y-1">
            <span className="text-xs font-black text-amber-600/80 uppercase tracking-wider">Tugas Perlu Dinilai</span>
            <h3 className="text-3xl font-black text-amber-700">{dashboardData.stats.pendingGrading}</h3>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-8 space-y-4">
          <div className="flex items-center justify-between select-none">
            ...
            <button
              ...
              className="px-3 py-1.5 bg-[#7047EB] hover:bg-[#5E3BD2] text-white text-[10px] font-black rounded-lg transition-colors cursor-pointer shadow-sm focus:outline-none"
            >
              + Tambah Tugas
            </button>
          </div>
            ...
            {dashboardData.classes.map((cls) => (
              ...
              >
                <div className="space-y-1.5 text-left">
                  ...
                  <h3 className="text-base font-extrabold text-slate-805 leading-snug">
                    {cls.name}
                  </h3>
        ...
        <div className="lg:col-span-4">
          <div className="bg-white border border-slate-100 rounded-2xl p-6 shadow-sm hover:shadow-md transition-shadow duration-200 space-y-5 h-full">
            <h2 className="text-sm font-black text-slate-500 uppercase tracking-wider pb-1 select-none">
            ...
            {dashboardData.recentSubmissions.length === 0 ? (
              <div className="py-12 text-center border border-dashed border-slate-100 rounded-xl bg-slate-50/50">
                <p className="text-xs font-black text-slate-400">Belum ada pengumpulan tugas baru.</p>
              </div>
            ) : (
              <div className="space-y-4 text-left select-none">
                {dashboardData.recentSubmissions.map((sub) => (
                  ...
                  >
                    ...
                    <div className="w-8 h-8 rounded-full bg-violet-100 text-[#7047EB] flex items-center justify-center font-black text-[10px] shrink-0">
                    ...
                    <div className="space-y-0.5 min-w-0 flex-1">
                      ...
                      <p className="text-[10px] text-slate-500 font-bold truncate leading-relaxed group-hover:text-[#7047EB] transition-colors" title={sub.assignmentTitle}>
                    ...
                    <ChevronRight className="w-4 h-4 text-slate-300 group-hover:text-[#7047EB] transition-colors shrink-0 self-center" />
                  </div>
                ))}
              </div>
            )}
