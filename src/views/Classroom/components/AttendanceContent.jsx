  ...
  return (
    <div className="space-y-6 text-left select-none">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
        ...
        <div className="bg-gradient-to-br from-[#7047EB] to-[#8e6cf2] text-white rounded-2xl p-6 shadow-sm flex flex-col justify-between">
          <div>
          ...
          {hasCheckedInToday ? (
            <div className="flex flex-col items-center justify-center py-4 text-center">
              ...
              <p className="text-xs text-slate-400 font-bold mt-1">
                Anda terdaftar sebagai <span className={`px-2 py-0.5 rounded-md text-[10px] font-black uppercase ${getStatusBadge(todayStatus)}`}>{todayStatus}</span> hari ini.
              </p>
            </div>
          ) : (
              ...
              {!showExcuseForm ? (
                <div className="flex flex-col sm:flex-row gap-3">
                  <button
                    ...
                    className="flex-1 flex items-center justify-center gap-2 bg-[#7047EB] hover:bg-[#5b35d5] text-white text-xs font-black py-3 rounded-xl transition-all shadow-sm shadow-indigo-100 hover:shadow-md cursor-pointer disabled:opacity-50"
                  >
                ...
                <form onSubmit={handleExcuseSubmit} className="space-y-3 pt-2 border-t border-slate-50">
                  <div className="flex items-center gap-3">
                    <span className="text-xs font-extrabold text-slate-650">Status:</span>
                  ...
                  <div className="space-y-1">
                    <textarea
                      ...
                      className="w-full text-xs font-medium border border-slate-200 rounded-xl p-3 focus:outline-none focus:border-[#7047EB] transition-colors"
                      rows={2}
                      maxLength={150}
                    />
                  </div>
                  ...
                  <div className="flex gap-2 justify-end">
                    ...
                    <button
                      type="submit"
                      disabled={isSubmitting}
                      className="px-4 py-1.5 bg-[#7047EB] hover:bg-[#5b35d5] text-white text-xs font-black rounded-lg cursor-pointer disabled:opacity-50"
                    >
                      Kirim Keterangan
                    </button>
                  </div>
        ...
        {[
          ...
          <div key={idx} className="bg-white border border-slate-100 rounded-xl p-4 flex items-center justify-between shadow-sm">
            <div>
              ...
              <p className="text-xl font-black text-slate-805 mt-1">{item.value}</p>
            </div>
            ...
            <table className="w-full text-xs font-medium text-slate-650">
              <thead>
                <tr className="border-b border-slate-100 text-slate-400 font-extrabold text-left">
                  <th className="pb-3 font-black uppercase tracking-wider text-[10px]">Tanggal Pertemuan</th>
                  <th className="pb-3 font-black uppercase tracking-wider text-[10px]">Status</th>
                  <th className="pb-3 font-black uppercase tracking-wider text-[10px]">Keterangan</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-50">
                {history.map((row) => (
                  <tr key={row.id} className="hover:bg-slate-50/30 transition-colors">
                    <td className="py-3.5 font-bold text-slate-805">{row.date}</td>
                    <td className="py-3.5">
                      <span className={`px-2.5 py-0.5 rounded-full text-[10px] font-black uppercase ${getStatusBadge(row.status)}`}>
                        {row.status}
                      </span>
                    </td>
